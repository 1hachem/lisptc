	...
	return (
		<Frame
			...
			spec={{
				...
				ceiling: "~60 nodes and ~200 edges, then read the matrix below instead",
				fails: "the nodes have no natural layer, leaving a prettier hairball",
			}}
			tags={["Sugiyama DAG", "node-link"]}
			why={
				<>
					A force layout of this many nodes is a hairball. Pinning every node to
					the layer the architecture already declares, then ordering within the
					layer to cut crossings, makes the same edges readable. A violation is
					an edge that climbs.
				</>
			}
		>
...
export function MatrixFrame({
	...
	return (
		<Frame
			...
			why={
				<>
					The same edges as a grid, axes ordered by layer. A clean architecture
					puts every mark on one side of the diagonal, so a violation is not an
					edge to hunt for in a tangle, it is a mark in the empty triangle.
				</>
			}
		>
