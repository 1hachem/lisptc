...
import { autolinkExtension } from "./autolink.ts";
...
function componentsFor(lisp: boolean) {
	return {
		...Object.fromEntries(
			TEXT_TAGS.map((name) => [name, tag(name, undefined, lisp)]),
		),
		...
		th: tag("th", "px-2 py-1 font-semibold", lisp),
		td: tag("td", "border-bg2/50 border-t px-2 py-1 align-top", lisp),
		hr: tag("hr", "my-2 border-bg2 border-t"),
	};
}

const LISP = componentsFor(true);
const PLAIN = componentsFor(false);

const highlightCode = createTanStackMarkdownHighlighter(highlighter);
const extensions = [streamingMarkdownExtension(), autolinkExtension()];
