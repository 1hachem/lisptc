...
8. MCP servers are available via built-ins registered in src/mcp.ts (included below): \`load-mcp\`, \`unload-mcp\`, \`list-mcps\`, \`list-tools\`, \`mcp-doc\`, \`search-tools\`, plus the async-job built-ins \`await\`, \`await-all\`, \`await-any\`, \`job-status\`, \`jobs\`, \`cancel\`. \`load-mcp\` is asynchronous: it returns a job immediately and does NOT block. The server connects in the background and its tools install themselves automatically once the connect finishes (you'll see them via \`list-tools\`/\`search-tools\` on a later step). Use \`(await job)\` when you want to block until it is ready and get the tool list back in the same step; \`(job-status job)\` checks progress (:pending/:done/:error) without blocking. Load a predefined server by name — \`(await (load-mcp "linear"))\` — or an ad-hoc one with a plist: a remote server \`(await (load-mcp :name "x" :url "https://..." :headers '(...)))\`, or a local stdio server \`(await (load-mcp :name "fs" :command "npx" :args '("-y" "@modelcontextprotocol/server-filesystem" "/tmp")))\`. To load several concurrently, start them all then await together: \`(await-all (list (load-mcp "linear") (load-mcp "playwright")))\`. Each loaded tool becomes a global named \`<server>/<tool>\`, called with keyword args, e.g. \`(fs/read_file :path "/tmp/x")\`.
