...
### Async jobs runtime (`src/jobs.ts`)
...
- The `JobsRuntime` **interface** (`call`/`start`/`awaitJob`/`awaitAll`/`awaitAny`/`jobStatus`/`cancelJob`/`onSettled`/`shutdown`) is the swappable transport; the bundled `LocalJobsRuntime` holds native promises plus a per-job `AbortController` in a map on this thread (a different backend, e.g. a Redis-backed queue, can implement the same interface without touching consumers). It takes a `Dispatch`, which is what keeps `jobs.ts` free of MCP. Alongside it: the `Job` handle (jobId + optional finalizer), and the `Jobs` manager which owns the runtime + the in-flight-job set, does `collect` (run a job's finalizer once, idempotent, reaping settled jobs), wires `onSettled` push events, and installs the generic job built-ins (`await`, `await-all`, `await-any`, `job-status`, `jobs`, `cancel`) via `installBuiltins`.
- **A `Promise` returned from a builtin is not a Lisp value** — the evaluator yields it and resumes with its result (`BuiltInFunc.settle`). Lisp code cannot construct one, so the rule is unambiguous, and a rejection becomes an `EvalException` whose *value* is the error text, so a `catch` binds something actionable. A builtin that needs to suspend inside its own body is declared with `defGen` and is itself a generator.
- **Job lifecycle:** a producer starts a background job via `runtime.start(op, payload)` and hands back a `Job` (tracked with `jobs.track`); results apply via a **finalizer** through two paths: (1) **push** — the runtime's `promise.then` fires and `Jobs` applies the finalizer, so effects appear **automatically between evals without an explicit await**, and now mid-eval at any suspension point too; (2) **`(await job)`** — returns the job's promise, so the evaluator suspends and the finalizer runs on the way back. The two are idempotent via `Job.finalized`. `(cancel job)` calls `AbortController.abort()`, wired into the op's `signal`, so it aborts the in-flight request. Timeouts (`DEFAULT_TIMEOUT_MS` 30s, `AWAIT_TIMEOUT_MS` 50s) are policy, not deadlock guards: nothing blocks the thread any more.

### MCP integration (`src/mcp.ts` + `src/mcp-client.ts`)
MCP is a **consumer of the jobs runtime**: `registerMcp` builds a `Jobs` over a `JobsRuntime` (default: a `LocalJobsRuntime` over `mcpDispatch`), installs the generic job built-ins via that `Jobs`, and adds the MCP built-ins on top. `src/mcp-client.ts` is MCP-only: it defines the domain `dispatch` (`connect`, `call-tool`, `disconnect`, `login`, `logout`, `authorize`, `list-tools`, `search`) and runs the SDK's clients on this thread. Its `clients` map, OAuth callback server and token store are module-scoped, so every interp in a process shares them.
