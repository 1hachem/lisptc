...
function useWatch(effet: () => void, deps: unknown[]) {
	...
	useEffect(() => {
		...
		if (avant.length === deps.length && avant.every((v, i) => v === deps[i]))
			return;
		effet();
		// biome-ignore lint/correctness/useExhaustiveDependencies: le tableau est celui du watcher, recompare a la main ci-dessus
	}, deps);
}
...
export function BloubBot({
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: la poignee ne lit que des references, elle n'a pas a se reconstruire
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: la boucle est montee une fois et lit les props par `p`
	useEffect(() => {
		if (!anime) return;
		/*
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: les fonctions posees en ecoute ne lisent que `l`
	useEffect(() => {
		if (!ecoute) return;
	 ...
	 */
	// biome-ignore lint/correctness/useExhaustiveDependencies: watcher sur `gaze` : le reste ne vient que de `l`
	useEffect(() => {
		if (gaze) {
			l.gazeSince = l.clock;
			l.scripted = true;
        ...
        */}
				...
				>
					...
					{frame.eyes.map((eye, i) => (
						<path
							// biome-ignore lint/suspicious/noArrayIndexKey: l'index EST l'identite — le moteur rend l'oeil gauche puis le droit, toujours
							key={i}
							d={eye.d}
							transform={
				...
				{frame.arcs.map((arc) => (
					...
					>
						{arc.grad.stops.map((c, i) => (
							<stop
								// biome-ignore lint/suspicious/noArrayIndexKey: un arret de degrade n'a pas d'identite hors de son rang, qui est aussi son offset
								key={i}
